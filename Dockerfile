...
# OPERATIONS.md §1/§4) and gems (the CLI, useful inside a running
# container for `docker compose exec <node> gems query /data "..."`).
#
...
WORKDIR /build
COPY . .
RUN cargo build --release --workspace --bin gems-cluster-node --bin gems
